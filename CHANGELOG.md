...
- **`LandCoverClassifier.export_model(path)`** writes what is needed to reproduce a supervised classification outside Earth Engine: `<path>.json` with the configuration of every processor (sensor, periods and dates, years, reducer, cloud and SAR options, ROI), the feature stack (indices per sensor, statistics, normalization, pixel size, CRS, resampling), the ordered features, the training setup (class property, train fraction, seed), the algorithm with the parameters actually used, the model as Earth Engine describes it (for random forests every tree as text, the importance and the out-of-bag error; for CART also a Graphviz `dot`) and the accuracy metrics; and `<path>_samples.csv` with one row per sample — coordinates, class, every feature and its train/validation split — ready to refit the model in scikit-learn or R. Training samples now keep their coordinates for this.
...
- **Classifiers built on different feature stacks could not be compared fairly**: `add_training_data()` drew the train/validation split after reading the stack, so the same points got a different split on every stack. The split is now drawn on the samples first, with a `seed` (default 0), and `add_training_data()` gains `train_fraction` (default 0.7, as before). The `train_fraction` argument of `classify_supervised()` never had any effect; it is kept for compatibility and warns when set.
